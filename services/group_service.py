...
class GroupService:
    def __init__(self):
        self.groups: Dict[int, Group] = {}  # user_id -> Group

    def get_group(self, user_id: int) -> Group:
        if user_id not in self.groups:
            self.groups[user_id] = Group()
        return self.groups[user_id]

    def add_member(self, user_id: int, character_name: str) -> bool:
    ...
    def remove_member(self, user_id: int, character_name: str) -> bool:
        group = self.get_group(user_id)
    ...
    def get_members(self, user_id: int) -> List[GroupMember]:
        group = self.get_group(user_id)
        return group.members

    def get_formatted_members(self, user_id: int) -> str:
        members = self.get_members(user_id)
        if not members:
